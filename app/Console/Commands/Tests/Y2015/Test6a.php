    ...
    public function getResult(array $inputs): int
    {
        ...
        foreach ($inputs as $input) {
            ...
            if (str_starts_with($input, 'turn on')) {
                $lights = $this->updateLights($lights, $matches[1], $matches[2], $matches[3], $matches[4], fn($lightValue) => true);
            } else if (str_starts_with($input, 'turn off')) {
                $lights = $this->updateLights($lights, $matches[1], $matches[2], $matches[3], $matches[4], fn($lightValue) => false);
            } else {
                $lights = $this->updateLights($lights, $matches[1], $matches[2], $matches[3], $matches[4], fn($lightValue) => !$lightValue);
            }
        }

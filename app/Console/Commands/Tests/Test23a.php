...
class Test23a implements Test
{
    ...
    public function getResult(array $inputs): int
    {
        ...
        preg_match('/([A-D])#([A-D])#([A-D])#([A-D])#/', $inputs[3], $matches2);
        $rooms = [
            [$matches[1], $matches2[1]],
            [$matches[2], $matches2[2]],
            [$matches[3], $matches2[3]],
            [$matches[4], $matches2[4]],
        ];
    ...
    private function getOutOfPlacePods(array $rooms): array
    {
        ...
        foreach ($rooms as $position => $room) {
            ...
            if ($pod0OutOfPlace || $pod1OutOfPlace) {
                if ($room[0] !== null && ($pod0OutOfPlace || $pod1OutOfPlace)) {
